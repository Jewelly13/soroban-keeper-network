...
use keeper_registry::{KeeperError, TaskStatus, TaskType, MAX_CALLDATA_LEN};
...
impl RegisterTaskInput {
    fn interpret(&self, env: &soroban_sdk::Env) -> (TaskType, Bytes, i128, u64, u32, u32) {
...
fuzz_target!(|data: &[u8]| {
    ...
    let (task_type, calldata, reward, deadline, ttl_ledgers, lock_ledgers) = input.interpret(env);
    ...
    match result {
        Ok(task_id) => {
            ...
            let task_id = task_id.expect("a registered task id must decode");

            // 2. Task should exist and be readable
            let task = client
                .try_get_task(&task_id)
                .expect("successfully registered task should be readable")
                .expect("task record must decode");
            ...
            assert_eq!(task.calldata, calldata, "Calldata should be stored exactly");
            ...
            assert!(
                task_count > 0,
                "Task count should increase after registration"
            );
        }
        Err(err) => {
            // Registration failed - verify it was a typed KeeperError rather
            // than a host-level trap.
            let err = err.expect("rejection must be a typed KeeperError, not a host trap");
